...
///
/// ImixSerialization is the kernel-level driver that provides
...
// application.
pub struct ImixSerialization<'a, U: UART + 'a> {
    uart: &'a U,
    app: TakeCell<App>,
    buffer: TakeCell<&'static mut [u8]>,
}

impl<'a, U: UART> ImixSerialization<'a, U> {
    pub fn new(uart: &'a U, buffer: &'static mut [u8]) -> ImixSerialization<'a, U> {
        ImixSerialization {
            uart: uart,
            app: TakeCell::empty(),
...
impl<'a, U: UART> Driver for ImixSerialization<'a, U> {
    ...
    /// Register a callback to the ImixSerialization driver.
    ///
    ...
    /// Issue a command to the ImixSerialization driver.
    ///
...
impl<'a, U: UART> Client for ImixSerialization<'a, U> {
